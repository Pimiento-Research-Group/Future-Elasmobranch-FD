###############################################################################################################
# 22. Environmental drivers of RCP 2.6 FD
## This R code analyses the relationship of environmental data in 2100 from RCP 2.6 AquaMaps 
...
# Load RCP 2.6 environmental data
...
FD_RCP26_full <- merge(final_map_df_RCP26_extinctions, RCP26_environ, by.x = "Grid", by.y = "CsquareCode", all.x = TRUE)

FD_RCP26 <- FD_RCP26_full %>% 
  select(Grid,centerlat,centerlong,sp_richn,fric,fori,fspe,FUn,
...
SalinityB2100c_Spp <- glm(sp_richn ~ SalinityB2100c, data = FD_RCP26, family = poisson)
...
PrimProd2100c_Spp <- glm(sp_richn ~ PrimProd2100c, data = FD_RCP26, family = poisson)
...
IceCon2100c_Spp <- glm(sp_richn ~ IceCon2100c, data = FD_RCP26, family = poisson)
...
OxyMean_Spp <- glm(sp_richn ~ OxyMean, data = FD_RCP26, family = poisson)
...
OxyB2100c_Spp <- glm(sp_richn ~ OxyB2100c, data = FD_RCP26, family = poisson)
...
SalinityB2100c_FRic <- glm(fric ~ SalinityB2100c, data = FD_RCP26, family = binomial)
...
PrimProd2100c_FRic <- glm(fric ~ PrimProd2100c, data = FD_RCP26, family = binomial)
...
IceCon2100c_FRic <- glm(fric ~ IceCon2100c, data = FD_RCP26, family = binomial)
...
OxyMean_FRic <- glm(fric ~ OxyMean, data = FD_RCP26, family = binomial)
...
OxyB2100c_FRic <- glm(fric ~ OxyB2100c, data = FD_RCP26, family = binomial)
...
SalinityB2100c_FUn <- glm(FUn ~ SalinityB2100c, data = FD_RCP26, family = binomial)
...
PrimProd2100c_FUn <- glm(FUn ~ PrimProd2100c, data = FD_RCP26, family = binomial)
...
IceCon2100c_FUn <- glm(FUn ~ IceCon2100c, data = FD_RCP26, family = binomial)
...
SalinityB2100c_FSp <- glm(fspe ~ SalinityB2100c, data = FD_RCP26, family = binomial)
...
PrimProd2100c_FSp <- glm(fspe ~ PrimProd2100c, data = FD_RCP26, family = binomial)
...
IceCon2100c_FSp <- glm(fspe ~ IceCon2100c, data = FD_RCP26, family = binomial)
...
OxyMean_FSp <- glm(fspe ~ OxyMean, data = FD_RCP26, family = binomial)
visreg(OxyMean_FSp, scale = "response", main = "", xlab = "Mean dissolved oxygen", ylab = "P(FSp)")
OxyMean_FSp_drop<- drop1(OxyMean_FSp, test = "Chi")

OxyB2100c_FSp <- glm(fspe ~ OxyB2100c, data = FD_RCP26, family = binomial)
