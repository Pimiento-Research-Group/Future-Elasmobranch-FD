...
load(file = "C:/Users/2022207/Dropbox/Jack's PhD/Chapter 3. Future shark FD/Analyses/Final paper R codes/data/AquaMaps/FD_RCP85_extinctions_map_df.RData")
load(file = "C:/Users/2022207/Dropbox/Jack's PhD/Chapter 3. Future shark FD/Analyses/Final paper R codes/data/AquaMaps/FD_RCP85_extinctions_Species_richness_map.RData")
...
FRic_shift_summary <- merged_data_RCP85ext_FD %>%
  group_by(FRic_Shift) %>%
  summarise(Count = n()) %>%
  mutate(Percentage = (Count / sum(Count)) * 100)

# Calculate percentages for FUn_Shift
FUn_shift_summary <- merged_data_RCP85ext_FD %>%
  group_by(FUn_Shift) %>%
  summarise(Count = n()) %>%
  mutate(Percentage = (Count / sum(Count)) * 100)

# Calculate percentages for FSp_Shift
FSp_shift_summary <- merged_data_RCP85ext_FD %>%
  group_by(FSp_Shift) %>%
  summarise(Count = n()) %>%
  mutate(Percentage = (Count / sum(Count)) * 100)
...
SR_breaks <- c(-149,-75,0,1,37,73)
FRic_breaks <- c(-0.76,-0.38,0,0.0000001,0.33,0.66)
FUn_breaks <- c(-0.27, -0.13, 0, 0.0000000001, 0.16, 0.32)
FSp_breaks <- c(-0.28, -0.14, 0, 0.00000001, 0.12, 0.23)

Map_RCP85ext_Ab_sprich <- ggplot() +
  geom_point(
...
save(Map_RCP85ext_Ab_sprich, file = "C:/Users/2022207/Dropbox/Jack's PhD/Chapter 3. Future shark FD/Analyses/Final paper R codes/data/AquaMaps/RCP85ext_Spp_Rich_absolute.RData")

Map_RCP85ext_Ab_fric <- ggplot() +
  geom_point(
...
save(Map_RCP85ext_Ab_fric, file = "C:/Users/2022207/Dropbox/Jack's PhD/Chapter 3. Future shark FD/Analyses/Final paper R codes/data/AquaMaps/RCP85ext_FRic_absolute.RData")

Map_RCP85ext_Ab_fun <- ggplot() +
  geom_point(
    data = merged_data_RCP85ext_FD,
...
save(Map_RCP85ext_Ab_fun, file = "C:/Users/2022207/Dropbox/Jack's PhD/Chapter 3. Future shark FD/Analyses/Final paper R codes/data/AquaMaps/RCP85ext_FUn_absolute.RData")

Map_RCP85ext_Ab_fsp <- ggplot() +
  geom_point(
    data = merged_data_RCP85ext_FD,
  ...
  scale_color_gradientn(
    ...
    limits = c(-0.28, 0.23),
    breaks = c(-0.28, -0.14, 0, 0.12, 0.23),
    labels = scales::label_number(accuracy = 0.01),
...
save(Map_RCP85ext_Ab_fsp, file = "C:/Users/2022207/Dropbox/Jack's PhD/Chapter 3. Future shark FD/Analyses/Final paper R codes/data/AquaMaps/RCP85ext_FSp_absolute.RData")
