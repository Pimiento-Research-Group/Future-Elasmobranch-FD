...
limits_PC1 <- c(-0.4, 0.3)
limits_PC2 <- c(-0.4, 0.3)
limits_PC3 <- c(-0.4, 0.3)
limits_PC4 <- c(-0.4, 0.3)
...
# Plot space 
Fig_S5a <- plot_grid(plot_sp_vert12,plot_sp_vert34,
                     labels = c("A","B"),
...
# What % of top 20, 50 & 100 FUSE species are present in AquaMaps? - Produces Table S2
...
plot_FUSE_rank_lollipop <- ggplot(
  FUSE_rank,
  aes(
    x     = Species,
    y     = FUSE,
    fill  = IUCN,
    shape = factor(AquaMaps)
  )
) +
  geom_segment(
    aes(xend = Species, yend = 0),
    size  = 1.5,
    color = "darkgrey"
  ) +
  geom_point(
