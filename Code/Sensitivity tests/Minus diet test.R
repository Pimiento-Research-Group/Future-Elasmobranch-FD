###############################################################################################################
# Sensitivity tests 6: Minus diet
## This R code conducts sensitivity tests based on individual trait removal
...
traits <- data %>% 
  select(Species,habitat,vertical,terrestriality,thermo,feeding,
         max.length,deepwater,body.plan,reproduction)
...
sharks_traits_cat <- tibble(trait_name = c("habitat","vertical","terrestriality","thermo","feeding",
                                           "max.length","deepwater","body.plan","reproduction"),
                            trait_type = c("N", "N","O", "O", "O", "Q", "N", "N", "N"))
...
res.TaxonVar<- lapply(1:10000, function(x){
  ...
  rownames(FD.metrics) <- c("sp_richn","fric")
...
FDmetrics_taxonvar.diet<- res_df %>% 
  select(Scenario:fric)
save(FDmetrics_taxonvar.diet,file = "C:/Users/Jack Cooper/Documents/iucn/iucn_sim/Robustness tests/Minus_diet/CritE_full_buffer.RData")

FDmetrics_long_TaxonVar.diet<- melt(FDmetrics_taxonvar.diet, id.vars= "Scenario")
save(FDmetrics_long_TaxonVar.diet,file = "C:/Users/Jack Cooper/Documents/iucn/iucn_sim/Robustness tests/Minus_diet/CritE_full_long_buffer.RData")
