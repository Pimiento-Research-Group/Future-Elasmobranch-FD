###############################################################################################################
# Sensitivity tests 10: Minus reproduction
...
traits <- data %>% 
  select(Species,habitat,vertical,terrestriality,thermo,feeding,
         diet,max.length,deepwater,body.plan)
...
sharks_traits_cat <- tibble(trait_name = c("habitat","vertical","terrestriality","thermo","feeding",
                                           "diet","max.length","deepwater","body.plan"),
                            trait_type = c("N", "N","O", "O", "O", "N", "Q", "N", "N"))
...
FDmetrics_taxonvar.reproduction<- res_df %>% 
  select(Scenario:fun)
save(FDmetrics_taxonvar.reproduction,file = "C:/Users/Jack Cooper/Documents/iucn/iucn_sim/Robustness tests/Minus_reproduction/CritE_full_buffer.RData")

FDmetrics_long_TaxonVar.reproduction<- melt(FDmetrics_taxonvar.reproduction, id.vars= "Scenario")
save(FDmetrics_long_TaxonVar.reproduction,file = "C:/Users/Jack Cooper/Documents/iucn/iucn_sim/Robustness tests/Minus_reproduction/CritE_full_long_buffer.RData")
