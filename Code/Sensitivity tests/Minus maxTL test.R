###############################################################################################################
# Sensitivity tests 7: Minus max TL
...
traits <- data %>% 
  select(Species,habitat,vertical,terrestriality,thermo,feeding,
         diet,deepwater,body.plan,reproduction)
...
sharks_traits_cat <- tibble(trait_name = c("habitat","vertical","terrestriality","thermo","feeding",
                                           "diet","deepwater","body.plan","reproduction"),
                            trait_type = c("N", "N","O", "O", "O", "N", "N", "N", "N"))
...
FDmetrics_taxonvar.TL<- res_df %>% 
  select(Scenario:fun)
save(FDmetrics_taxonvar.TL,file = "C:/Users/Jack Cooper/Documents/iucn/iucn_sim/Robustness tests/Minus_maxTL/CritE_full_buffer.RData")

FDmetrics_long_TaxonVar.TL<- melt(FDmetrics_taxonvar.TL, id.vars= "Scenario")
save(FDmetrics_long_TaxonVar.TL,file = "C:/Users/Jack Cooper/Documents/iucn/iucn_sim/Robustness tests/Minus_maxTL/CritE_full_long_buffer.RData")
